...
使い始める手順は、(1) このcrate一式を作業用にコピー、(2) `templates/*.rs` の一つを `src/main.rs` へコピー、(3) TODOの問題固有部分を書く、です。AtCoder提出時は `cargo bundle` で、参照している `src/` moduleだけを単一ファイルへinlineできます。
...
各templateは標準ライブラリだけの`Input`/`Output`を使います。入力が必要な値だけを読み、出力の最後に明示的にflushします。

```rust
let mut input = Input::new();
let n: usize = input.read();
let values: Vec<i64> = input.vec(n);

let mut output = Output::new();
output.join_space(values);
output.flush();
```

`Input`は`has_next`, `read`, `vec`, `matrix`, `string`, `chars`, `bytes`, `usize1`を提供します。`bytes`はtokenを文字列へ変換せず、そのままのバイト列として取得し、`usize1`は0を明示的にpanicします。`Output`は`print`, `println`, `join`, `join_space`, `join_line`, `yes_no`, `flush`を提供します。`join`はseparatorを要素間に置き、空iterを含め必ず末尾へ改行を出します。OutputはDrop時にもflushしますが、提出前などで即時に出力したいときは`flush()`を呼んでください（明示flushの失敗はpanicします）。
...
- `Input` / `Output`: 外部crateなしのtoken入力・buffered出力。テンプレートとexamplesはこれを使用します。
...
`cargo bundle` は `src/main.rs` の `use atcoder_heuristic::...` をRustの構文木として解析し、実際に参照した自作moduleとその依存先だけを単一ファイルへinlineします。`#[cfg(test)]` とrustdocを除去し、生成コードをCargoで再コンパイルしてから `target/bundle.rs` へ保存します。macOSでは成功時にクリップボードへもコピーします。

```bash
cargo bundle
```

クリップボードを変更しない場合や出力先を指定する場合は次を使います。
...
バンドラー自体は `tools/bundler/` に分離されており、提出コードへ依存crateを持ち込みません。問題ディレクトリへ `template/` crate一式をコピーし、選んだ `templates/*.rs` を `src/main.rs` に置けばそのまま利用できます。

## Validation

通常の検証は `cargo test --all-targets` です。Random/Timer/SA/Beam/BitSet/Zobristに加え、TSPとLight Gridの差分評価・apply後の集計整合性も確認します。

バンドラーは `cargo test --manifest-path tools/bundler/Cargo.toml` で検証します。`Input` / `Output` のroot re-exportを含む生成bundleを再びCargoでcompileし、提出用inlineコードとしても利用できることを確認します。
